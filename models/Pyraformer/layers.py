...
def get_q_k(input_size, window_size, stride, device):
    ...
    for i in range(input_size):
        mask[i, 0:window_size] = i + \
            torch.arange(window_size) - window_size // 2
    ...
    for i in range(second_length):
        mask[input_size+i, 0:window_size] = input_size + \
        ...
        mask[input_size+i, mask[input_size+i] >
             fourth_start - 1] = fourth_start - 1
    for i in range(third_length):
        mask[third_start+i, 0:window_size] = third_start + \
            i + torch.arange(window_size) - window_size // 2
        ...
        mask[third_start+i, mask[third_start+i]
             > full_length - 1] = full_length - 1
    for i in range(fourth_length):
        mask[fourth_start+i, 0:window_size] = fourth_start + \
            i + torch.arange(window_size) - window_size // 2
    ...
    def __init__(self, d_model, d_inner, n_head, d_k, d_v, dropout=0.1, normalize_before=True, use_tvm=False, q_k_mask=None, k_q_mask=None):
        ...
        if use_tvm:
            self.slf_attn = PyramidalAttention(
                n_head, d_model, d_k, d_v, dropout=dropout, normalize_before=normalize_before, q_k_mask=q_k_mask, k_q_mask=k_q_mask)
        else:
...
class Decoder(nn.Module):
    ...
    def forward(self, x_dec, x_mark_dec, refer):
        ...
        dec_enc, _ = self.layers[1](
            dec_enc, refer_enc, refer_enc, slf_attn_mask=mask)
